...
@login_required
@audit_view('CASE_FILES_VIEW', 'Officer accessed Evidence Locker')
def case_files(request):
    """
    Evidence Locker - National Security Case Files Repository
    Shows all investigation records with download capabilities
    """
    from .models import AuditTrail
    
    user_role = get_user_role(request.user)
    
    # Get all analysis records for current officer
    case_records = AnalysisRecord.objects.filter(
        officer=request.user
    ).select_related('officer').order_by('-created_at')
    
    # Get forensic report logs
    forensic_logs = ForensicReportLog.objects.filter(
        generated_by=request.user
    ).order_by('-timestamp')
    ...
    context = {
        ...
        'critical_cases': case_records.filter(risk_score__gte=75).count(),
        'ongoing_cases': case_records.filter(investigation_status='ONGOING').count(),
    }
    
    return render(request, 'dashboard/case_files.html', context)


@login_required
def live_audit_feed(request):
    """
    Live Audit Log API - Real-time system activity feed
    Returns last 20 audit trail entries
    """
    from .models import AuditTrail
    
    try:
        # Get last 20 audit entries across all users (for national security monitoring)
        recent_audits = AuditTrail.objects.all().order_by('-timestamp')[:20]
        
        audit_feed = []
        for audit in recent_audits:
            # Parse action to create readable messages
            message = ""
            if 'GEMMA' in audit.action:
                message = f"🤖 Gemma AI analysis completed by {audit.username}"
            elif 'SMURFING' in audit.description.upper():
                message = f"🚨 Smurfing pattern detected - Case flagged"
            elif 'STRUCTURING' in audit.description.upper():
                message = f"⚠️ Structuring attempt identified"
            elif 'PDF_EXPORT' in audit.action:
                message = f"📄 Forensic report generated by {audit.username}"
            elif 'BATCH_UPLOAD' in audit.action:
                message = f"📊 Batch analysis initiated by {audit.username}"
            elif 'CRITICAL' in audit.description.upper():
                message = f"🔴 Critical risk alert - Immediate review required"
            else:
                message = f"{audit.action.replace('_', ' ').title()} - {audit.username}"
            
            audit_feed.append({
                'timestamp': audit.timestamp.strftime('%H:%M:%S'),
                'message': message,
                'user': audit.username,
        ...
        logger.error(f"Failed to fetch audit feed: {str(e)}", exc_info=True)
        return JsonResponse({'success': False, 'error': str(e)}, status=500)
