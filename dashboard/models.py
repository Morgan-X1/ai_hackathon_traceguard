...
class AnalysisRecord(models.Model):
    """
    Enhanced Analysis Records for full accountability and audit trail.
    
    Stores every Gemma Deep Analysis performed by officers for compliance tracking,
    case history, and report generation. Enables officers to track their investigations
    and generate professional PDF reports for FRC compliance audits.
    """
    # Officer accountability
    officer = models.ForeignKey(User, on_delete=models.PROTECT, related_name='analysis_records')
    
    # Investigation details
    target_account = models.CharField(max_length=100, help_text="Account ID under investigation")
    ...
    case_reference = models.CharField(max_length=50, blank=True, help_text="Optional case reference number")
    transaction_count = models.IntegerField(default=0, help_text="Number of transactions analyzed")
    total_amount_usd = models.DecimalField(max_digits=20, decimal_places=2, default=0, help_text="Total transaction volume")
    
    # Typologies and risk factors
    ...
    def __str__(self):
        return f"Analysis: {self.target_account} by {self.officer.get_full_name() or self.officer.username} - {self.created_at.date()}"
    
    def get_officer_name(self):
        """Return officer's full name or fallback to username"""
        return self.officer.get_full_name() or self.officer.username
